...
const AddNode = ({ setNodes }) => {
  ...
  return (
    <div className="flex items-center justify-between p-4 ">
      <div className="text-sm text-[#555] max-md:w-[60%]">
        To Delete a Component/Node Select it and press Backspace key
      </div>
      <button className="gray-button mr-10 max-md:mr-2" onClick={() => setAddNode(true)}>
        Add Node
      </button>
      {addNode && (
        <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-50 z-[1000] ">
          <div className="bg-white w-[600px] p-6 rounded-xl flex flex-col justify-between content-between gap-5">
            <div>
            ...
            <div>
              ...
              <button onClick={() => setAddNode(false)} className="gray-button">
                Cancel
              </button>
            </div>
          </div>
